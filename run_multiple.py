...
def main():
    ...
    target_path = "./main_" + args.simulation + ".py"

    output_csv = "time_and_error_" + args.simulation + f"_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}" + ".csv"
    ...
    with open(output_csv, 'w') as f:
        ...
        for run in range(args.N):
            ...
            result = subprocess.run(["/usr/bin/env", "python3", target_path],
            ...
            writer.writerow({'time': time, 'error': err})
...
def process_output(output):
    ...
    if time_match:
        time = 3600.0 * float(time_match.group(1)) + 60.0 * float(
            time_match.group(2)) + float(time_match.group(3))
    else:
